...
async function generateResponse(messageContent, chatId) {
    try {
        ...
        const memoryContext = JSON.stringify(await searchKnowledgeBase(messageEmbedding, chatId))

        const recentMessages = await messageModel.find({ chatId }).sort({ createdAt: -1 }).limit(4);
