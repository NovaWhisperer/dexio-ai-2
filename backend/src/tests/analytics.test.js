...
beforeAll(async () => {
    ...
    userId = user._id
    userToken = jwt.sign({ id: user._id, role: user.role }, JWT_SECRET, { expiresIn: "7d" })
    ...
    adminId = admin._id;
    adminToken = jwt.sign({ id: admin._id, role: admin.role }, JWT_SECRET, { expiresIn: "7d" });
...
describe("Analytics Routes", () => {
    ...
    describe('GET /v1/analytics/:userId', function () {
        ...
        it('return 200 Admin cookie, valid userId, analytics document exists → 200', async () => {
            ...
            const userAnalytics = await userAnalyticsModel.findOne({ userId: userId })

            expect(userAnalytics.chatCount).toBe(0)

            expect(res.body.data.message).toBe("User analytics data fetched successfully")
        });
    });
